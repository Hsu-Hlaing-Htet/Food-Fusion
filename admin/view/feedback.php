...
$total_query = $conn->prepare("SELECT COUNT(*) AS total FROM contacts WHERE subject = 'Feedback'");
$total_query->execute();
...
<div class="flex-grow flex overflow-hidden">
    ...
    <div class="w-3/4 p-6 overflow-y-auto bg-[#fcfaf7]">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="export.php?export_type=feedbacks" class="px-3 py-2 border border-[#800020]/20 rounded-lg hover:bg-[#800020]/5">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-[#800020]/10 overflow-hidden">
                ...
                <table class="w-full">
                    <thead class="bg-[#EBE6DA]">
                        <tr>
                            ...
                            <th class="p-3 text-left text-sm font-semibold text-[#800020]">Feedback</th>
                    ...
                    <tbody class="divide-y divide-[#800020]/10">
                        <?php foreach ($feedback_entries as $feedback): ?>
                            <tr class="hover:bg-[#800020]/5 transition-colors">
                                ...
                                <td class="p-3 text-sm"><?= $feedback['id'] ?></td>
                                <td class="p-3 font-medium"><?= htmlspecialchars($feedback['name']) ?></td>
                                <td class="p-3 text-sm"><?= htmlspecialchars($feedback['email']) ?></td>
                                <td class="p-3 text-sm max-w-xs">
                                    <div class="line-clamp-2"><?= htmlspecialchars($feedback['message']) ?></div>
                                </td>
                                <td class="p-3 text-sm"><?= $feedback['created_at'] ?></td>
                                <td class="p-3 flex gap-2">
                                    <a href="view_feedback.php?id=<?= $feedback['id'] ?>" class="text-blue-600 hover:text-blue-800">
            ...
            <div class="p-4 border-t border-[#800020]/10 flex justify-between items-center">
                <div class="text-sm text-gray-600">
                    Showing <?= ($page - 1) * $per_page + 1 ?>-<?= min($page * $per_page, $total_feedback) ?> of <?= $total_feedback ?> feedback entries
                </div>
