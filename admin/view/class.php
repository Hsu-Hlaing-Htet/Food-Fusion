...
$section_id = 'cookingclass'; // Hardcoded as per requirement
...
// Get total cards for this section
$total_query = $conn->prepare("SELECT COUNT(*) AS total FROM content_cards WHERE section_id = ?");
$total_query->bind_param("s", $section_id);
$total_query->execute();
$total_result = $total_query->get_result();
$total_cards = $total_result->fetch_assoc()['total'];
$total_pages = ceil($total_cards / $per_page);

// Get cards data for cookingclass section
$stmt = $conn->prepare("
    ...
    LIMIT ? OFFSET ?
");
$stmt->bind_param("sii", $section_id, $per_page, $offset);
$stmt->execute();
        ...
        <div class="flex justify-between items-center mb-6">
            <nav aria-label="breadcrumb">
                <ol class="flex space-x-2 text-[#800020]">
                    ...
                    <li class="breadcrumb-item"><a href="content.php" class="hover:text-[#600018]">Content Cards /</a></li>
                    <li class="breadcrumb-item active">Cooking Class</li>
                </ol>
            </nav>
            <a href="create_card.php?section=cookingclass" class="bg-[#800020] text-white px-4 py-2 rounded-lg hover:bg-[#600018]">
                + New
            </a>
        </div>
                ...
                <table class="w-full table-auto">
                    <thead class="bg-[#EBE6DA]">
                        <tr>
                            ...
                            <th class="p-3 text-left text-sm font-semibold text-[#800020]">Link</th>
                            <th class="p-3 text-left text-sm font-semibold text-[#800020]">Status</th>
                            ...
                            <td class="p-3 text-sm">
                                <?php if ($card['link_url']): ?>
                                    <a href="<?= htmlspecialchars($card['link_url']) ?>" 
                                       class="text-[#800020] hover:underline truncate max-w-xs block"
                                       target="_blank">
                                        <?= htmlspecialchars($card['link_url']) ?>
                                    </a>
                                <?php endif; ?>
                            </td>
                            <td class="p-3 text-sm">
