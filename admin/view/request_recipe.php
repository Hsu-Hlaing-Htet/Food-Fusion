...
// Get total number of recipe requests
$total_query = $conn->prepare("SELECT COUNT(*) AS total FROM contacts WHERE subject = 'Recipe Request'");
$total_query->execute();
$total_result = $total_query->get_result();
$total_requests = $total_result->fetch_assoc()['total'];
$total_pages = ceil($total_requests / $per_page);

// Get recipe requests data
$stmt = $conn->prepare("
        ...
        <div class="flex justify-between items-center mb-6">
            <nav aria-label="breadcrumb">
                <ol class="flex space-x-2 text-[#800020]">
                    ...
                    <li class="breadcrumb-item active">Recipe Requests</li>
                </ol>
            </nav>
            <a href="export.php?export_type=request_recipe" class="px-3 py-2 border border-[#800020]/20 rounded-lg hover:bg-[#800020]/5">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-[#800020]/10 overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-[#800020]/10">
                        <?php foreach ($recipe_requests as $request): ?>
                            <tr class="hover:bg-[#800020]/5 transition-colors">
                                ...
                                <td class="p-3 font-medium"><?= htmlspecialchars($request['name']) ?></td>
                                <td class="p-3 text-sm"><?= htmlspecialchars($request['email']) ?></td>
                                <td class="p-3 text-sm max-w-xs">
                                    <div class="line-clamp-2"><?= htmlspecialchars($request['message']) ?></div>
                                    <?php if (strlen($request['message']) > 100): ?>
                                        <a href="view_request.php?id=<?= $request['id'] ?>" class="text-[#800020] hover:underline mt-1 block">
                                            Read more
                                        </a>
                                    <?php endif; ?>
                                </td>
                                <td class="p-3 text-sm"><?= $request['created_at'] ?></td>
                                <td class="p-3 flex gap-2">
                                    <a href="view_request.php?id=<?= $request['id'] ?>" class="text-blue-600 hover:text-blue-800">
            ...
            <div class="p-4 border-t border-[#800020]/10 flex justify-between items-center">
                <div class="text-sm text-gray-600">
                    Showing <?= ($page - 1) * $per_page + 1 ?>-<?= min($page * $per_page, $total_requests) ?> of <?= $total_requests ?> requests
                </div>
